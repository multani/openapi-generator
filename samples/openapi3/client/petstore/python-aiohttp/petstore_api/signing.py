...
class HttpSigningConfiguration:
    ...
    :param signed_headers: A list of strings. Each value is the name of a HTTP header
        ...
        included in :attr:`signed_headers`.
        ...
        When :attr:`signed_headers` contains the ``Digest`` value, the client performs the
        following operations:

        1. Calculate a digest of request body, as specified in
            `RFC3230, section 4.3.2 <https://datatracker.ietf.org/doc/html/rfc3230#section-4.3.2>`_.
    ...
    :param hash_algorithm: The hash algorithm for the signature. Supported values are
        ...
        If the :attr:`signing_scheme` is ``rsa-sha256``, the hash algorithm must be set
        to ``None`` or ``sha256``.

        If the :attr:`signing_scheme` is ``rsa-sha512``, the hash algorithm must be set
        to ``None`` or ``sha512``.
    :param signature_max_validity: The signature max validity, expressed as
        a :class:`datetime.timedelta` value. It must be a positive value.
    """
